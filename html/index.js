...
//     //     if (pushed_object_ids.includes(obj)) continue;
...
function getPushedObjects(pushworld, state, displacement) {
    ...
    const actor = pushworld.moveables[0]; //actor is object
    const tangible_objects = [];
    tangible_objects.push(...pushworld.moveables); //objects
    tangible_objects.push(...pushworld.walls); //objects
    const moving_parts = [actor];
    const pushed_object_ids = []; //stores IDs
    let transitive_stopping = false;
    const id_to_anchor_pos = getObjectIDsToPositions(pushworld, state)

    while (moving_parts.length > 0 && !transitive_stopping) {

        const obj = moving_parts.pop(); //returns the last index array of moving parts and remove it from moving_parts

        if (pushed_object_ids.includes(obj.id)) continue; //skip already moved, usually if duplicates and not processed yet
        ...
        for (const other_obj of tangible_objects) {
            ...
            if (transitive_stopping) break;

            if (pushed_object_ids.includes(other_obj.id)) continue;

            if (other_obj.id == 'aw' && obj.id != 'a') continue;

            const obj_new_pos = addPoints(id_to_anchor_pos[obj.id], displacement);//6.3
            const other_obj_rel_pos = subPoints(obj_new_pos, id_to_anchor_pos[other_obj.id]); //6.3 - 4,3 = 2,0


            for (const local_obj_boundary_pixel of obj.boundaryPixels) {

                const convert_to_local = addPoints(local_obj_boundary_pixel, other_obj_rel_pos); //0,0 + 2,0 = 2,0

                if (is2DPointInArray(convert_to_local, other_obj.boundaryPixels)) { //2,0 in 0,0 0,1 1,0 1,1 

                    if (other_obj.id == 'w') {

                        transitive_stopping = true;

                    } else if (other_obj.id == 'aw' && obj.id == 'a') {

                        transitive_stopping = true;

                    } else {

                        moving_parts.push(other_obj);
